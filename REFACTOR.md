...
| # | Item | Fixes | Size |
|---|------|-------|------|
| R1 | **Honest docs** — align README/docstrings with the implementation; move Rotation Policy to roadmap; delete or wire up `kb/state/current_layout.json` | F1 | S |
| R2 | **Typed domain objects** — `Title` dataclass + `Rating` enum with construction-time validation; normalize ESRB names → codes at the `game_catalog` boundary; gates consume the typed layout (and keep re-checking as defense in depth) | F5 | M |
...
| R4 | **Capability split** — `BaseAgent` → KB-grounding base + LLM mixin; `ValidationAgent` gets the KB base only, making validator determinism structural | F3 | S |
