...
| R3 | **Loud policy loader** — single `PolicyLoader` that parses rating policy, required fields, row-size bounds, and the tier list (CLI choices) from the KB; missing file / failed parse warns loudly (or fails, configurable); code defaults demoted to clearly-labeled emergency fallback | F4 | M | — |
